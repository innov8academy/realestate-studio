...
function extractParametersFromSchema(
  ...
  // Improve labels for first-frame / last-frame image inputs
  // When a model has both a "start" and "end" style image input,
  // relabel them to "First Frame" / "Last Frame" for clarity.
  const imageInputNames = new Set(inputs.filter(i => i.type === "image").map(i => i.name));
  const FIRST_FRAME_NAMES = ["first_frame", "start_image", "image_url"];
  const LAST_FRAME_NAMES = ["last_frame", "end_image", "tail_image_url", "end_image_url"];
  const hasFirstAndLast =
    FIRST_FRAME_NAMES.some(n => imageInputNames.has(n)) &&
    LAST_FRAME_NAMES.some(n => imageInputNames.has(n));

  if (hasFirstAndLast) {
    for (const input of inputs) {
      if (input.type !== "image") continue;
      if (FIRST_FRAME_NAMES.includes(input.name)) input.label = "First Frame";
      else if (LAST_FRAME_NAMES.includes(input.name)) input.label = "Last Frame";
    }
  }
...
function getKieSchema(modelId: string): ExtractedSchema {
  ...
  const schemas: Record<string, ExtractedSchema> = {
    ...
    // Kling 3.0 — array-based: image_urls[0] + image_urls[1]
    "kling-3.0/video": {
      parameters: [
        { name: "aspect_ratio", type: "string", description: "Output aspect ratio", enum: ["16:9", "9:16", "1:1"], default: "16:9" },
        { name: "duration", type: "string", description: "Video duration", enum: ["5", "10"], default: "5" },
        { name: "mode", type: "string", description: "Generation mode", enum: ["pro", "standard"], default: "pro" },
      ],
      inputs: [
        { name: "prompt", type: "text", required: false, label: "Prompt" },
    ...
    // Kling 2.1 Pro — separate keys: image_url + tail_image_url
    "kling/v2-1-pro": {
      parameters: [
        { name: "aspect_ratio", type: "string", description: "Output aspect ratio", enum: ["16:9", "9:16", "1:1"], default: "16:9" },
        { name: "duration", type: "string", description: "Video duration", enum: ["5", "10"], default: "5" },
    ...
    // Seedance 1.5 Pro — array-based: input_urls[0] + input_urls[1]
    "bytedance/seedance-1.5-pro": {
      parameters: [
        { name: "aspect_ratio", type: "string", description: "Output aspect ratio", enum: ["16:9", "9:16", "1:1"], default: "16:9" },
        { name: "resolution", type: "string", description: "Output resolution", enum: ["720p", "1080p"], default: "720p" },
        { name: "duration", type: "string", description: "Video duration in seconds", enum: ["4", "8", "12"], default: "8" },
      ],
      inputs: [
        { name: "prompt", type: "text", required: false, label: "Prompt" },
        { name: "first_frame", type: "image", required: true, label: "First Frame" },
    ...
    // Seedance 1.0 Lite — separate keys: image_url + end_image_url
    "bytedance/v1-lite-image-to-video": {
      ...
      inputs: [
        { name: "prompt", type: "text", required: false, label: "Prompt" },
        { name: "image_url", type: "image", required: true, label: "First Frame" },
        { name: "end_image_url", type: "image", required: false, label: "Last Frame" },
      ],
    },
    // Hailuo 02 Pro — separate keys: image_url + end_image_url
    "hailuo/02-image-to-video-pro": {
      parameters: [
        { name: "prompt_optimizer", type: "boolean", description: "Enable prompt optimization", default: true },
      ],
      inputs: [
        { name: "prompt", type: "text", required: false, label: "Prompt" },
        { name: "image_url", type: "image", required: true, label: "First Frame" },
    ...
    // Hailuo 02 Standard — separate keys: image_url + end_image_url
    "hailuo/02-image-to-video-standard": {
      parameters: [
        { name: "duration", type: "string", description: "Video duration in seconds", enum: ["6", "10"], default: "6" },
        { name: "resolution", type: "string", description: "Output resolution", enum: ["768P", "1080P"], default: "768P" },
      ],
      inputs: [
        { name: "prompt", type: "text", required: false, label: "Prompt" },
        { name: "image_url", type: "image", required: true, label: "First Frame" },
...
export async function GET(
  ...
  // Catch-all [...modelId] yields an array of path segments (e.g. ["veo3-fast", "image-to-video"])
  const { modelId: modelIdSegments } = await params;
  const decodedModelId = modelIdSegments.map(s => decodeURIComponent(s)).join("/");
