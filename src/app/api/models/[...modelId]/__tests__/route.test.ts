...
describe("/api/models/[modelId] schema endpoint", () => {
  ...
  describe("isImageInput classification", () => {
    it("should NOT classify boolean params with 'image' in name as image inputs", async () => {
      ...
      const response = await GET(request, { params: Promise.resolve({ modelId: modelId.split("/") }) });
  ...
  describe("caching", () => {
    it("should return cached results on subsequent requests", async () => {
      ...
      const response1 = await GET(request1, { params: Promise.resolve({ modelId: modelId.split("/") }) });
      ...
      const response2 = await GET(request2, { params: Promise.resolve({ modelId: modelId.split("/") }) });
