...
function getKieImageInputKey(modelId: string): string {
  ...
  // Kling 2.1 Pro uses singular image_url
  if (modelId === "kling/v2-1-pro") return "image_url";
  // Seedance 1.0 Lite uses singular image_url
  if (modelId === "bytedance/v1-lite-image-to-video") return "image_url";
  // Hailuo 02 models use singular image_url
  if (modelId === "hailuo/02-image-to-video-pro" || modelId === "hailuo/02-image-to-video-standard") return "image_url";
  // Kling 3.0 and Seedance 1.5 Pro use virtual names → handled by FRAME_COMBINE_MAP
...
async function generateWithKie(
    ...
    for (const [key, value] of Object.entries(input.dynamicInputs)) {
      if (value !== null && value !== undefined && value !== '') {
        ...
        if (typeof value === 'string' && value.startsWith('data:image')) {
          ...
          if (key === "image_url" || key === "video_url" || key === "tail_image_url" || key === "end_image_url" || key === "first_frame" || key === "last_frame") {
            inputParams[key] = url;
          } else {
            inputParams[key] = [url];
          }
  ...
  // Combine virtual first_frame/last_frame into the actual API array field
  // Array-based models use virtual names in the schema so we get 2 separate handles,
  // then we merge them into the real API field here before sending.
  const FRAME_COMBINE_MAP: Record<string, string> = {
    "kling-3.0/video": "image_urls",
    "bytedance/seedance-1.5-pro": "input_urls",
    "veo3/image-to-video": "imageUrls",
    "veo3-fast/image-to-video": "imageUrls",
  };

  const combineField = FRAME_COMBINE_MAP[modelId];
  if (combineField) {
    const firstFrame = inputParams["first_frame"];
    const lastFrame = inputParams["last_frame"];
    delete inputParams["first_frame"];
    delete inputParams["last_frame"];

    const frames: string[] = [];
    if (firstFrame) frames.push(firstFrame as string);
    if (lastFrame) frames.push(lastFrame as string);
    if (frames.length > 0) {
      inputParams[combineField] = frames;
      handledImageKeys.add(combineField);
    }
  }
  ...
  if (isVeoModel(modelId)) {
    ...
    // Handle both virtual frame names (from FRAME_COMBINE_MAP) and direct imageUrls
    if (inputParams.imageUrls) {
      const imageUrlsArray = Array.isArray(inputParams.imageUrls)
        ? inputParams.imageUrls as string[]
        : [inputParams.imageUrls as string];
      veoBody.imageUrls = imageUrlsArray;
      // Set generationType for start+end frame (2 images)
