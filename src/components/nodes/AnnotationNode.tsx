...
export function AnnotationNode({ id, data, selected }: NodeProps<AnnotationNodeType>) {
  ...
  // Reactively get connected upstream image (same pattern as ImageCompareNode)
  const connectedImage = useMemo(() => {
    const incomingEdge = edges.find(
      (edge) => edge.target === id && (edge.targetHandle === "image" || !edge.targetHandle)
    );
    ...
    // Extract image from different source node types
  ...
  // Auto-update sourceImage when connected upstream image changes
  // Skip during workflow execution to avoid interfering with parallel node execution
  useEffect(() => {
    if (isRunning) return;
    if (connectedImage && connectedImage !== nodeData.sourceImage) {
      updateNodeData(id, { sourceImage: connectedImage });
      // If no annotations yet, also pass through as output
      if (!nodeData.annotations || nodeData.annotations.length === 0) {
        updateNodeData(id, { outputImage: connectedImage });
      }
    }
  }, [connectedImage, id, updateNodeData, nodeData.sourceImage, nodeData.annotations, isRunning]);
