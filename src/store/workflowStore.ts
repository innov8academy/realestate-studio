...
export const useWorkflowStore = create<WorkflowStore>((set, get) => ({
  ...
  validateWorkflow: () => {
    ...
    // Check generateVideo nodes have at least one input (text or image)
    // I2V models can run with just an image — prompt is optional
    nodes
      ...
      .forEach((node) => {
        ...
        if (!textConnected && !imageConnected) {
          errors.push(`Video node "${node.id}" missing text or image input`);
        }
      });
      ...
      switch (node.type) {
          ...
          case "videoStitch": {
            ...
            try {
              ...
              // Build FormData for server-side stitching
              const formData = new FormData();
              for (let i = 0; i < videoBlobs.length; i++) {
                formData.append(`video-${i}`, videoBlobs[i], `clip_${i}.mp4`);
              }
              formData.append("loopCount", String(nodeData.loopCount || 1));
              if (nodeData.speedPreset) {
                formData.append("speedPreset", nodeData.speedPreset);
              }

              updateNodeData(node.id, { progress: 20 });
              ...
              if (!response.ok) {
                const errBody = await response.json().catch(() => ({ error: "Stitch failed" }));
                throw new Error(errBody.error || `Stitch failed (${response.status})`);
              }

              updateNodeData(node.id, { progress: 80 });
  ...
  regenerateNode: async (nodeId: string) => {
    const { nodes, updateNodeData, getConnectedInputs, isRunning, currentNodeIds } = get();

    // Block if a full workflow execution is in progress (executeWorkflow sets isRunning + _abortController)
    if (isRunning && get()._abortController) {
      logger.warn('node.execution', 'Cannot regenerate node, workflow execution in progress', { nodeId });
      return;
    }

    // Block if this specific node is already running
    ...
    // Add this node to currentNodeIds (allows multiple nodes to run in parallel)
    set({ isRunning: true, currentNodeIds: [...get().currentNodeIds, nodeId] });

    // Helper: remove this node from currentNodeIds; clear isRunning when no nodes left
    const finishNode = () => {
      const remaining = get().currentNodeIds.filter((id) => id !== nodeId);
      set({ currentNodeIds: remaining, isRunning: remaining.length > 0 });
    };
    ...
    try {
      if (node.type === "generateImage") {
        ...
        // For I2V models, prompt is optional — only block if BOTH prompt and images are missing
        const hasPrompt = text || dynamicInputs.prompt || dynamicInputs.negative_prompt;
        const hasDynamicImages = Object.entries(dynamicInputs).some(
          ([, v]) => v && typeof v === 'string' && (v.startsWith('data:image') || v.startsWith('http'))
        );
        if (!hasPrompt && images.length === 0 && !hasDynamicImages) {
          logger.error('node.error', 'generateVideo regeneration failed: missing inputs', {
            nodeId,
          });
          updateNodeData(nodeId, {
            status: "error",
            error: "Missing required inputs - connect an image or prompt",
          });
          finishNode();
          await logger.endSession();
          return;
        ...
        try {
          ...
          updateNodeData(nodeId, { progress: 10 });
          ...
          updateNodeData(nodeId, { progress: 20 });
          ...
          updateNodeData(nodeId, { progress: 80 });
